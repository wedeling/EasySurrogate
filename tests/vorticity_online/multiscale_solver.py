...
if store:
    ...
    for q in range(Q):
        ...
        if '_hat' in QoI[q]:
            # store Fourier coefs
            samples[QoI[q]] = np.zeros(
                [S, 2 * np.int(Ncutoff_LF) + 1, np.int(Ncutoff_LF) + 1], dtype='complex')
            # store full fields
...
# restart from a previous stored state (set restart = True, and set t to
# the end time of previous simulation)
if restart:
...
for n in range(n_steps):
    ...
    w_hat_np1_HF, VgradW_hat_n_HF = get_w_hat_np1(
        w_hat_n_HF, w_hat_nm1_HF, VgradW_hat_nm1_HF, P, norm_factor)
    ...
    w_hat_np1_LF, VgradW_hat_n_LF = get_w_hat_np1(
        w_hat_n_LF, w_hat_nm1_LF, VgradW_hat_nm1_LF, P_LF, norm_factor_LF, sgs_hat=r_hat_nm1)
...
if not plot:
    ...
    ax = fig.add_subplot(121, xlabel=r'x', ylabel=r'y', title='t = ' +
                         str(np.around(t / day, 2)) + ' days')
    ct = ax.contourf(
        x, y, np.fft.irfft2(w_hat_np1_HF), vmin=np.min(
            np.fft.irfft2(w_hat_np1_HF)), vmax=np.max(
            np.fft.irfft2(w_hat_np1_HF)))
    plt.colorbar(ct)

    ax = fig.add_subplot(122, xlabel=r'x', ylabel=r'y', title='t = ' +
                         str(np.around(t / day, 2)) + ' days')
    ct = ax.contourf(
        x,
        y,
