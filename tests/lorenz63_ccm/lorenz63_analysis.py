...
fig = plt.figure(figsize=[8, 4])
ax1 = fig.add_subplot(121, ylabel=r'$\mathrm{ACF}\;X$', xlabel='time')
ax2 = fig.add_subplot(122, ylabel=r'$\mathrm{ACF}\;Y$', xlabel='time')

acf_X_ref = np.zeros(acf_lag - 1)
acf_X_sol = np.zeros(acf_lag - 1)
acf_Y_ref = np.zeros(acf_lag - 1)
acf_Y_sol = np.zeros(acf_lag - 1)
...
ax1 = fig.add_subplot(111, ylabel=r'$\mathrm{CCF(X,Y)}$', xlabel='time')
