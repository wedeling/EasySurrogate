...
lags = [range(1, 21), range(1, 21)]
max_lag = np.max(list(chain(*lags)))

X_train, y_train = feat_eng.lag_training_data([X, Y], Z, lags=lags)
...
mean_Z = np.mean(y_train, axis=0)
std_Z = np.std(y_train, axis=0)
...
for i in range(max_lag):
    feat_eng.append_feat([[X[i]], [Y[i]]], max_lag)
