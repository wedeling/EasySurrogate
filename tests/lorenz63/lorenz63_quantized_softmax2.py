...
def rhs_surrogate(X_n, s=10, r=28):
    ...
    f_n[2] = sampler.resample(idx_max)[0]
...
def step_with_surrogate(X_n, f_nm1):
    ...
    feat_eng.append_feat([[X_np1[0]], [X_np1[1]]], max_lag)
...
lags = [range(1, 2, 1), range(1, 2, 1)]
max_lag = np.max(list(chain(*lags)))

X_train, y_train = feat_eng.lag_training_data([X[:, 0], X[:, 1]], X_dot[:, 2], lags = lags)
...
surrogate = es.methods.ANN(X=X_train, y=feat_eng.y_idx_binned, n_layers=3, 
                           ...
                           activation='hard_tanh', batch_size=128, loss = 'cross_entropy',
...
plot_lorenz(ax1, X[:, 0], X[:, 1], X[:, 2], title='Lorenz63 data')
plot_lorenz(ax2, X_surr[:, 0], X_surr[:, 1], X_surr[:, 2], title='Lorenz63 neural network')
...
ax = fig.add_subplot(131, xlabel=r'$x$')
...
ax = fig.add_subplot(132, xlabel=r'$y$')
X_dom_surr, X_pde_surr = post_proc.get_pde(X_surr[0:-1:10, 1])
...
ax = fig.add_subplot(133, xlabel=r'$z$')
X_dom_surr, X_pde_surr = post_proc.get_pde(X_surr[0:-1:10, 2])
X_dom, X_pde = post_proc.get_pde(X[0:-1:10, 2])
ax.plot(X_dom, X_pde, 'ko', label='L63')
ax.plot(X_dom_surr, X_pde_surr, label='ANN')
plt.yticks([])
plt.legend(loc=0)
...
R_sol = post_proc.auto_correlation_function(X_surr[:, 1], max_lag = 500)
dom_acf = np.arange(R_data.size)*dt
ax.plot(dom_acf, R_data, 'ko', label='L63')
ax.plot(dom_acf, R_sol, label='ANN')
leg = plt.legend(loc=0)
...
R_data = post_proc.auto_correlation_function(X[:, 2], max_lag = 500)
R_sol = post_proc.auto_correlation_function(X_surr[:, 2], max_lag = 500)
dom_acf = np.arange(R_data.size)*dt
ax.plot(dom_acf, R_data, 'ko', label='L63')
ax.plot(dom_acf, R_sol, label='ANN')
leg = plt.legend(loc=0)
