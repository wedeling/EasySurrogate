...
if not load:
    surrogate = es.methods.RNN(X_train, feat_eng.y_idx_binned, alpha = 0.001, loss = 'cross_entropy',
                               ...
                               n_out = n_bins*n_softmax, n_softmax = n_softmax, training_mode='offline',
...
n_pred = n_train
X_surr = np.zeros([n_pred, 3])
X_surr_dot = np.zeros([n_pred, 3])
