...
surrogate = es.methods.GP_Surrogate(backend='scikit-learn')
print('Time to initialise the surrogate: {:.3} s'.format(t.time() - time_init_start))

time_train_start = t.time()
surrogate.train(features, target, test_frac=0.95, basekernel='Matern', noize='fit')
...
surrogate.train_sequentially(features, target, n_iter=10, acquisition_function='poi')
print('Indices of runs used for training: {}'.format(surrogate.feat_eng.train_indices))
surrogate.model.print_model_info()
#


campaign.add_app(name='gp_campaign', surrogate=surrogate)
campaign.save_state(file_path='skl_gem_500_wf_1405_opt.pickle')
