...
acf_B_ref = np.zeros(acf_lag-1); acf_B_sol = np.zeros(acf_lag-1)
...
ccf_X_ref = np.zeros(acf_lag-1); ccf_X_sol = np.zeros(acf_lag-1)
ccf_B_ref = np.zeros(acf_lag-1); ccf_B_sol = np.zeros(acf_lag-1)

#average over all spatial points
for k in range(K-1):
    print('k=%d'%k)

    ccf_X_ref += 1/K*analysis.cross_correlation_function(X_ref[start_idx:, k], X_ref[start_idx:,k+1], max_lag=1000)
    ccf_X_sol += 1/K*analysis.cross_correlation_function(X_qsn[start_idx:, k], X_qsn[start_idx:, k+1], max_lag=1000)

    ccf_B_ref += 1/K*analysis.cross_correlation_function(B_ref[start_idx:, k], B_ref[start_idx:,k+1], max_lag=1000)
    ccf_B_sol += 1/K*analysis.cross_correlation_function(B_qsn[start_idx:, k], B_qsn[start_idx:, k+1], max_lag=1000)
