...
class CCM_analysis(BaseAnalysis):
    ...
    def plot_2D_binning_object(self):
        ...
        ax.plot(self.ccm_surrogate.feats[:,0], self.ccm_surrogate.feats[:,1], '+', color='lightgray', alpha=0.3)
        ax.vlines(self.ccm_surrogate.bins[0], np.min(self.ccm_surrogate.feats[:,1]), np.max(self.ccm_surrogate.feats[:,1]))
        ax.hlines(self.ccm_surrogate.bins[1], np.min(self.ccm_surrogate.feats[:,0]), np.max(self.ccm_surrogate.feats[:,0]))
       
        ax.plot(self.ccm_surrogate.x_mid_pad_tensor[:,0], self.ccm_surrogate.x_mid_pad_tensor[:,1], 'g+')

        #plot the mapping
        for i in range(self.ccm_surrogate.max_binnumber):
            ax.plot([self.ccm_surrogate.x_mid_pad_tensor[i][0], self.ccm_surrogate.x_mid_pad_tensor[self.ccm_surrogate.mapping[i]][0]], \
                    [self.ccm_surrogate.x_mid_pad_tensor[i][1], self.ccm_surrogate.x_mid_pad_tensor[self.ccm_surrogate.mapping[i]][1]], 'b', alpha=0.6)
